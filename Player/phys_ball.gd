...
func _init(ball_type := GolfBall.BallType.STANDARD):
	if ball_type == GolfBall.BallType.PREMIUM:
		_drag_mult = 0.9
		_lift_mult = 1.1
...
func _ball_type_changed(value):
	if value == GolfBall.BallType.PREMIUM:
		_drag_mult = 0.9
		_lift_mult = 1.1
	else:
		_drag_mult = 1.0
...
func _physics_process(delta: float) -> void:
	...
	var total_force := BallPhysics.calculate_forces(linear_velocity, omega, was_on_ground, params)
	var total_torque := BallPhysics.calculate_torques(linear_velocity, omega, was_on_ground, params)
	...
	# Check for rest
	if linear_velocity.length() < 0.1 and state != GolfBall.BallState.REST:
		_enter_rest_state()
	...
	if collision:
		...
		if _is_ground_normal(normal):
			...
			if is_landing:
				...
				var params := _create_physics_params()
				var bounce_result := BallPhysics.calculate_bounce(linear_velocity, omega, normal, state, params)
				linear_velocity = bounce_result.new_velocity
				omega = bounce_result.new_omega
				state = bounce_result.new_state

				print("  Velocity after bounce: ", linear_velocity, " (%.2f m/s)" % linear_velocity.length())
				on_ground = false
			else:
				on_ground = true
			...
			linear_velocity = linear_velocity.bounce(normal) * 0.30
	else:
		# No collision - check rolling continuity
		if state != GolfBall.BallState.FLIGHT and was_on_ground and position.y < 0.02 and linear_velocity.y <= 0.0:
			if should_debug and not on_ground:
				print("  NO COLLISION: setting on_ground=true (pos.y=%.4f, vel.y=%.2f)" % [position.y, linear_velocity.y])
			on_ground = true
		else:
			on_ground = false
			floor_normal = Vector3.UP
...
func _print_impact_debug() -> void:
	...
	print("  Velocity at impact: ", linear_velocity, " (%.2f m/s)" % linear_velocity.length())
...
func hit_from_data(data: Dictionary) -> void:
	...
	linear_velocity = Vector3(speed_mps, 0, 0) \
