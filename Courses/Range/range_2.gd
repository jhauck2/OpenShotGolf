...
func _ready() -> void:
	...
	$RangeCamera.look_at($Player.ball.position)
...
func set_camera_follow_mode(value: bool) -> void:

	if value:
		$RangeCamera.follow($Player.ball)
		$RangeCamera.setMode(1) # OBJECT
	else:
		$RangeCamera.follow(null)
		$RangeCamera.setMode(0) # NONE

func reset_camera_to_start() -> void:

	# Temporarily disable follow mode
	$RangeCamera.setMode(0)

	# Tween camera back to starting position
	var start_pos : Vector3 = $RangeCamera.followDistance  # Starting camera offset from ball at origin
	var tween := create_tween()
	tween.set_trans(Tween.TRANS_CUBIC)
	tween.set_ease(Tween.EASE_IN_OUT)
	tween.tween_property($RangeCamera, "global_position", start_pos, 1.5)

	await tween.finished

	# Reset ball to starting position
	$Player.ball.reset()
	$RangeCamera.setMode(1)
	$RangeCamera.follow($Player.ball)
